...
func startLoggingHttpServer(port int, staticRoot string, writers []HttpRequestWriter) {
	...
	go func() {
		err := http.ListenAndServe(net.JoinHostPort("0.0.0.0", strconv.Itoa(port)), router)
		if err != nil {
...
func startAdminHttpServer(port int, staticRoot string, redisClient redis.Conn) {
	...
	router.HandleFunc("/", HomeHandler(redisClient))
...
func HomeHandler(redisClient redis.Conn) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		params := struct {
			Title string
			Bins  []string
...
func LogHandler(handlers []HttpRequestHandler, writers []HttpRequestWriter) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, writer := range writers {
			err := writer.WriteHttpRequest(r.Context(), request)
			if err != nil {
				fmt.Println(err)
			}
		}
